    ...
    <div className="fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex mt-4 gap-2">
            {
                bgColors.map((color) => (
                    <button key={color} className="w-4 h-4   rounded-full ring cursor-pointer" style={{backgroundColor:color}} onClick={() => setBackground(color)}/>
                ))
            }

        </div>
        <div onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null)}} className="flex gap-2 mt-4">
            ...
            onClick={() => toast.promise(handleCreateStory(),{
             ...
             className="flex items-center justify-center gap-2 text-white py-3 mt-4 w-full rounded bg-linear-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition"> 
                <Sparkles size={18}/>
                 Create Story
               </button>
