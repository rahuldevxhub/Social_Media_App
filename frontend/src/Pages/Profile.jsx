              ...
              <div className="flex flex-wrap mt-6 max-w-6xl">
                {
                  posts.filter((post)=>post.image_urls.length >0).map((post) => (
                    ...
                    {post.image_urls.map((image,index)=>(
                      ...
                      <img src={image} alt="" key={index} className="w-64 aspect-video object-cover mb-2 mr-2 rounded"  />
                      <p className="absolute bottom-2 right-2 text-xs p-1 px-3  backdrop-blur-xl text-white opacity-0 group-hover:opacity-100 transition duration-300 ">Posted {moment(post.createdAt).fromNow()}</p>
                      </Link>
                    ))}
                    </>
                  ))
